...
class VendorsModel extends Model	{
	 ...
	 protected $primaryKey = 'vendorr_id';
    ...
    public function AllvendorsDropdown(){
	  $all_vendors     = $this->db->table('vendors')->get()->getResult(); 
	  $final_list       = array();
  ...
  public function update_vendor($data, $vendor_id){
		$this->db->table('vendors')->where('vendor_id', $vendor_id)->update($data);
		return 1;
	}
	
	public function update_vendor_image($data, $vendor_id){
		$this->db->table('vendors')->where('vendor_id', $vendor_id)->update($data);
		return 1;
	}

	public function add_vendor ($data){
	    $exists =  $this->db->table('vendors')->where('phone_no', $data['phone_no'])->where('LOWER(full_name)', strtolower($data['full_name']))->get()->getRowArray();
	    if(!$exists){
		...
		foreach($result as $values)
			{
			   ...
			   if($values['photo_path']!=''){
				   $show_image = base_url().'/public/documents/vendors/'.$values['photo_path'];
			   }
			   else{
				  $show_image  = base_url().'/public/documents/placeholder.png';
			   }
			   
			   $del_path       = base_url().'/vendors/deleteRecord/'.$values['vendor_id'];
			   
			   
			    $user_actions = '<a  href="'.base_url().'/vendors/edit/'.$values['vendor_id'].'"><span class="badge bg-primary">
