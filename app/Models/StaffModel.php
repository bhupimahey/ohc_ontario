...
class StaffModel extends Model	{
	...
	public function update_staff($data, $account_id){
		$this->db->table('users')->where('account_id', $account_id)->update($data);
		return 1;
	}
	
	public function update_staff_image ($data, $account_id){
		$this->db->table('users')->where('account_id', $account_id)->update($data);
		return 1;
	}
	...
	public function add_staff($data){
	    ...
	    $is_exists = $this->db->table('users')->where('mobile_no', $data['mobile_no'])->where('account_type', '3')->get()->getRowArray(); 
	    if($is_exists)
	      return '0';
	    else{
		...
		foreach($result as $values)
			{
			   ...
			   if($values['photo_path']!=''){
				   $show_image = base_url().'/public/documents/staff/'.$values['photo_path'];
			   }
			   else{
				  $show_image  = base_url().'/public/documents/placeholder.png';
			   }
			   
			   $del_path       = base_url().'/staff/deleteRecord/'.$values['account_id'];
			    $user_actions = '<a  href="'.base_url().'/staff/edit/'.$values['account_id'].'"><span class="badge bg-primary">
			...
			$user_status ='<span class="badge rounded-pill badge-light-danger me-1">Left</span>';
			
				$full_name = ucwords($values['first_name']).' '.ucwords($values['last_name']);
			    $records["data"][] = array(			
                      '<img src="'.$show_image.'" width="50px;" height="50px;" style="border-radius: 100px;">',				
					  $full_name,
					  '$'.$values['hourly_rate'],
					  $values['mobile_no'],
...
public function deleteRecord($id){
		$this->db->table('users')->where('account_id', $id)->where('account_type', '3')->delete();
		return 1;
	}
